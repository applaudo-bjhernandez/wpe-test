...
final class WPRSS_FTP_Display {
    ...
    public static function init() {
        ...
        if (function_exists('wpra_is_audio_file')) {
            // Add the content filter function that adds the audio player
            add_filter( 'the_content', array( 'WPRSS_FTP_Display', 'audio_player' ), $wprss_ftp_pcfp, 1 );
        }
        ...
        if (function_exists('rank_math')) {
            add_action('wp_head', function () {
                if (is_single()) {
                    // RankMath unhooks WordPress Core's wp_head action handlers. It triggers its own `rank_math/head` action
                    // during `wp_head` and attaches callbacks to that hook instead.
                    // Here, we are removing RankMath's canonical link printer function from its custom head action
                    remove_action('rank_math/head', [rank_math()->head, 'canonical'], 20);
                    // Then we re-attach WordPress Core's original canonical link printer, which F2P integrates with via the
                    // `get_canonical_url` filter.
                    add_action('rank_math/head', 'rel_canonical');
                }
            }, PHP_INT_MIN);
        }
    ...
    public static function post_content( $content ) {
        ...
        if (!empty($wp_query->queried_object)) {
            if ($wp_query->queried_object instanceof WP_Post_Type) {
                $post_type = $wp_query->queried_object;
            } elseif (property_exists($wp_query->queried_object, 'post_type')) {
        ...
        if (!empty($source)) {
            ...
            if ( isset($options['source_link']) && isset($options['source_link_text']) &&
                 WPRSS_FTP_Utils::multiboolean($options['source_link']) &&
    ...
    /**
     * Displays an audio player in the content of an imported post, if the option is enabled in the feed source.
     *
     * @since 3.14
     *
     ...
     */
    public static function audio_player($content)
    {
        global $post;

        if (!is_singular() && !is_feed() && !is_archive()) {
            return $content;
        }

        $meta = WPRSS_FTP_Meta::get_instance();

        $feedId = $meta->get_meta($post->ID, 'feed_source');
        if (empty($feedId)) {
            return $content;
        }

        $enabled = $meta->get_meta($feedId, 'audio_player');
        $url = $meta->get_meta($post->ID, 'enclosure_link');

        if (empty($enabled) || !WPRSS_FTP_Utils::multiboolean($enabled) || !wpra_is_audio_file($url)) {
            return $content;
        }
...
HTML;
        ...
        $position = $meta->get_meta($feedId, 'audio_player_pos');
        if ($position === 'before') {
            $content = $audioPlayer . $content;
        } else {
            $content = $content . $audioPlayer;
        }
