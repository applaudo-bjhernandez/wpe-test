...
class WPRSS_FTP_Images
{
    ...
    public function find_images($content, $source)
    {
        ...
        $srcUrl = get_post_meta($source, 'wprss_url', true);
        $srcUrlInfo = wpra_parse_url($srcUrl);
        $imgHost = $srcUrlInfo['scheme'] . '://' . $srcUrlInfo['host'];

        foreach ($foundImages as $idx => $url) {
            if (strpos($url, '/') === 0) {
                $foundImages[$idx] = $imgHost . $url;
            }
        }
    ...
    public function save_images($images, $postId, $source)
    {
        ...
        foreach ($images as $url) {
            ...
            if ($saveImages && !wprss_ftp_is_url_local($url)) {
                ...
                if ($this->image_obeys_minimum_size($newUrl, $min_width, $min_height) === false) {
                    $logger->warning('Skipped small image: {url}', ['url' => $newUrl]);
                    continue;
                }

                $img = wprss_ftp_media_sideload_image($newUrl, $postId);
        ...
        if (WPRSS_FTP_Utils::multiboolean($options['save_images_locally']) === true) {
            ...
            foreach ($finalImages as $old => $new) {
    ...
    public function save_ft_image_for_post($post_ID, $source, $images)
    {
        ...
        switch ($options['featured_image']) {
            ...
            case 'thumb':
            case 'enclosure':
                $isEnclosure = ($options['featured_image'] == 'enclosure');
                // Prepare the meta key in which to look for the image
                $metaKey = $isEnclosure
                    ? 'enclosure:thumbnail'
                    : 'media:thumbnail';

                $filteredMetaKey = apply_filters(
                    'wprss_ftp_featured_image_meta_key',
                    $metaKey,
                    $post_ID,
                    $source,
                 ...
                 */
                $useMetaPrefix = ($filteredMetaKey === $metaKey);
                $thumbnail = trim(WPRSS_FTP_Meta::get_instance()->get_meta($post_ID, $filteredMetaKey, $useMetaPrefix));

                // If getting the image from the enclosure and the metadata is empty, also check the enclosure link
                if (empty($thumbnail) && $isEnclosure) {
                    $thumbnail = trim(WPRSS_FTP_Meta::get_instance()->get_meta($post_ID, 'enclosure_link'));
                }
