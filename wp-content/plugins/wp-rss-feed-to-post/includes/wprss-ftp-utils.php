...
final class WPRSS_FTP_Utils {
    ...
    /**
     * Checks if audio player features are supported.
     *
     * @since 3.14
     *
     * @return bool
     */
    public static function is_audio_player_supported()
    {
    ...
    public static function is_multisite() {
        return function_exists('is_multisite') && function_exists('wp_get_sites') && is_multisite();
    }
    ...
    public static function is_multisite_and_main_site() {
        if (static::is_multisite() === false) {
            return false;
        }

        if (count(static::wp_get_sites()) === 0) {
            return __(
    ...
    public static function get_namespaces_from_feed() {
        ...
        $response = wp_remote_get($feed_source);

        // Show an error
        if ( is_wp_error($feed_source) ) {
            /** @var WP_Error $feed_source */
            die(
                sprintf(
                    __( 'Failed to read feed source XML: %s', WPRSS_TEXT_DOMAIN ),
                    $feed_source->get_error_message()
                )
            );
        }
