...
/*===== CONSTRUCTOR AND SINGLETON GETTER ==================================================================*/
	...
	public function disable_visual_editor($use_visual_editor) {
        ...
        if (is_object($screen) && $screen->base == 'post' && $screen->post_type == 'wprss_feed') {
...
#=== META BOXES REGISTRATION =============================================================================
	...
	public function add_meta_boxes() {
		...
		if (WPRSS_FTP_Utils::is_audio_player_supported()) {
            add_meta_box(
                'wprss-ftp-audio-metabox',
                __( 'Audio', WPRSS_TEXT_DOMAIN ),
...
#=== META BOXES AND META FIELDS =============================================================================
	...
	public function get_meta_fields( $what = '', $source_id = null ) {
		$fields = array(
            ...
            #== Attachments Metabox fields ===================
            'audio' => [
                'audio_player' => [
                    'label' => __('Show audio player', WPRSS_TEXT_DOMAIN),
                    'type' => 'checkbox',
                    'default' => 'false',
                ],
                'audio_player_pos' => [
                    'label' => __('Audio player position', WPRSS_TEXT_DOMAIN),
                    'type' => 'dropdown',
                    'options' => [
                        'before' => __('Before the content', WPRSS_TEXT_DOMAIN),
                        'after' => __('After the content', WPRSS_TEXT_DOMAIN),
                    ]
                ]
            ],
			...
			'extraction'	=>	array(
				'extraction_rules'	=>	array(
					...
					'manip_options'		=>	array(
                        ...
                        'remove_attr'   		=>	__( 'Remove the attribute', WPRSS_TEXT_DOMAIN ),
...
#== META BOX RENDERERS =================================================================================
	...
	public function render_audio_metabox($object, $box) { $this->render_metabox('audio', $object, $box); }
...
#== META DATA SAVING ===================================================================================
	...
	public function save_post_meta( $post_id, $post ) {
	    ...
	    if (get_post_type($post_id) !== 'wprss_feed') {
	        return;
        }
		...
		if ( $post_id && ! empty( $_POST[ self::META_PREFIX.'post_taxonomy' ] ) ) {
			...
			$whole_words = isset( $_POST[ self::META_PREFIX.'whole_words' ] )? $_POST[ self::META_PREFIX.'whole_words' ] : array_fill( 0, $n + 1, "false" );
			...
			for ( $i = 0; $i <= $n; $i++ ) {
				...
				$tax['whole_words'] = isset($whole_words[$i]) ? $whole_words[$i] : '';
