            ...
            <div class="ftp-tax-terms-wrapper wpra-metabox-row">
                ...
                <div class="ftp-tax-row-buttons">
					...
					<div class="ftp-taxonomy-conditions-wrapper wpra-metabox-row">
                        ...
                        <div class="ftp-whole-words-wrapper wpra-metabox-row">
                            <?php
                            $whole_words = isset($meta['whole_words']) ? $meta['whole_words'] : 'false';
                            echo WPRSS_FTP_Utils::boolean_to_checkbox(
                                $whole_words,
                                array(
                                    'id'		=>	'ftp-whole-words-'.$id,
                                    'class'		=>	'ftp-whole-words',
                                    'name'		=>	WPRSS_FTP_Meta::META_PREFIX . "whole_words[$id]",
                                    'value'		=>	'true',
                                )
                            );
                            ?>
                            <label for="ftp-whole-words-<?php echo $id; ?>">
                                <?php echo __('Match whole words only.', 'wprss') ?>
                            </label>
                        </div>
                    </div>
			<?php endif; ?>
 ...
 *
 * @since 3.1
 * @deprecated This function is only used by the deprecated {@link wprss_ftp_add_taxonomies_to_post()} and
 *             {@link wprss_ftp_process_auto_created_terms()} functions.
 *
 * @uses wp_insert_term
 * @uses term_exists
...
function wprss_ftp_create_or_get_term( $slug, $taxonomy, $return = 'SLUG', $args = array() ) {
	...
	if (is_wp_error($exists)) {
	    wpra_get_logger()->error('Failed to create term $0: $1', [$slug, $exists->get_error_message()]);
	    return false;
    }
 ...
 *
 * @since 3.1
 * @deprecated Use {@link RebelCode\Wpra\FeedToPost\Taxonomies\applyTerms()} instead
 * @see RebelCode\Wpra\FeedToPost\Taxonomies\applyTerms()
 *
 ...
 *
 * @since 3.5
 * @deprecated Use {@link RebelCode\Wpra\FeedToPost\Taxonomies\RuleCondition::isSatisfiedByItem()} instead.
 * @see RebelCode\Wpra\FeedToPost\Taxonomies\RuleCondition::isSatisfiedByItem()
 *
 ...
 *
 * @since 3.1
 * @deprecated This function is only used by the deprecated {@link wprss_ftp_add_taxonomies_to_post()} function.
 *
